...
def conv_bn_mish(in_c, out_c, kernel_size, stride, padding):
    return [
...
class MUnetBlock(nn.Module):
    ...
    def forward(self, up_in, x_in):
        ...
        x = self.actn(self.conv1(cat_x))
        x = self.actn(self.conv2(x))
        return self.bn(x)
...
class DynamicMUnet(nn.Module):
    ...
    def forward(self, x):
        ...
        if not hasattr(self, 'middle_conv'):
            ...
            middle_conv = nn.Sequential(*conv_bn_mish(middle_in_c, middle_in_c * 2, 3, 1, 1),
                                        *conv_bn_mish(middle_in_c * 2, middle_in_c, 3, 1, 1))
            self.middle_conv = middle_conv.type(self.encoder[0][0].weight.dtype).cuda()
        ...
        if not hasattr(self, 'upmodel'):
            ...
            for idx in self.sfs_idxs[::-1]:
                ...
                unet_block = MUnetBlock(up_in_c, x_in_c).type(self.encoder[0][0].weight.dtype).cuda()
                upmodel.append(unet_block)
                x_copy = unet_block(x_copy, self.sfs[idx].features)
                self.upmodel = nn.Sequential(*upmodel).type(self.encoder[0][0].weight.dtype).cuda()

            if imsize != self.sfs_szs[0][-2:]:
                extra_in_c = self.upmodel[-1].conv2.out_channels
                self.extra_block = nn.ConvTranspose2d(extra_in_c, extra_in_c, 2, 2).type(self.encoder[0][0].weight.dtype).cuda()

            final_in_c = self.upmodel[-1].conv2.out_channels
            self.final_conv = nn.Conv2d(final_in_c, self.n_classes, 1).type(self.encoder[0][0].weight.dtype).cuda()
